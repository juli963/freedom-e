...
#define DEF_InDivOut DEF_Inwidth/DEF_Outwidth
...
void check_fifos(Big_to_small_TB *tb, std::queue<uint64_t>* input_data, std::queue<uint64_t>* output_data, std::queue<uint8_t>* input_strb, std::queue<uint8_t>* output_strb, std::queue<uint8_t>* input_error, std::queue<uint8_t>* output_error, std::queue<uint8_t>* input_valid, std::queue<uint8_t>* output_valid, 
...
int main(int argc, char **argv) {
        ...
        const uint16_t Divider = DEF_InDivOut;
        const bool debug = false;

        Big_to_small_TB *tb = new Big_to_small_TB();
        ...
        printf("Conversion Parameters: Inbytes=%i, Outbytes=%i, Divider(In/Out)=%i\n", DEF_Inbytes, DEF_Outbytes, DEF_InDivOut);
        ...
        /*
                val data_in = Input(Vec(Inwidth/8, UInt(8.W)))
                val strb_in = Input(Vec(Inwidth/8, Bool()))
                val error_in = Input(Vec(Inwidth/8, Bool()))
                val valid_in = Input(Bool())
                val data_out = Output(Vec(Outwidth/8, UInt(8.W)))
                val strb_out = Output(Vec(Outwidth/8, Bool()))
                val error_out = Output(Vec(Outwidth/8, Bool()))
        ...
        for(uint16_t i=0; i<(fifo_size*Divider)+10; i++){
                ...
                for(x = 0; x < DEF_Outbytes;x++){
                        if( tb->m_core->io_valid_out > 0 ){
                                printf("Data @idx i=%i @Elem x=%i Data should: 0x%02lX Data is: 0x%02X || Strb should: %i Strb is: %i || Err should: %i Err is: %i  \n", i, x, output_data.front(), *(data_out[x]), output_strb.front(), *(strb_out[x]), output_error.front(), *(error_out[x]));
                                if(*(data_out[x]) != output_data.front()){
                                        CRED
                                        printf("Fail Data @idx i=%i @Elem x=%i Data should: 0x%02lX Data is: 0x%02X \n", i, x, output_data.front(), *(data_out[x]));
                                        CDEFAULT
                                        //exit;
                                }
                                if(*(strb_out[x]) != output_strb.front()){
                                        CRED
                                        printf("Fail Strobe @idx i=%i @Elem x=%i Data should: %i Data is: %i \n", i, x, output_strb.front(), *(strb_out[x]));
                                        CDEFAULT
                                        //exit;
                                }
                                if(*(error_out[x]) != output_error.front()){
                                        CRED
                                        printf("Fail Error @idx i=%i @Elem x=%i Data should: %i Data is: %i \n", i, x, output_error.front(), *(error_out[x]));
                                        CDEFAULT
                                        //exit;
                                }
                                if(tb->m_core->io_valid_out != output_valid.front()){
                                        CRED
                                        printf("Fail Valid @idx i=%i @Elem x=%i Data should: %i Data is: %i \n", i, x, output_valid.front(), tb->m_core->io_valid_out);
                                        CDEFAULT
                                        //exit;
                                }
...
void fill_fifos( std::queue<uint64_t>* input_data, std::queue<uint64_t>* output_data, std::queue<uint8_t>* input_strb, std::queue<uint8_t>* output_strb, std::queue<uint8_t>* input_error, std::queue<uint8_t>* output_error, std::queue<uint8_t>* input_valid, std::queue<uint8_t>* output_valid, bool withDebug ){
        ...
        uint8_t x,cnt= 0;
        const uint16_t bitmask_out = ((1<<DEF_Outbytes)-1);
        for(uint16_t i = 0; i<create_rand(100,200); i++){
                temp64_in = 0;
                temp64_out = 0;
                for(x= 0; x<DEF_Inbytes; x++){
                        ...
                        temp64_in |= ((uint64_t)temp8<<(DEF_Inwidth-8));
                        temp64_out |= ((uint64_t)temp8<<(DEF_Outwidth));
                        temp64_out >>= 8;
                        //temp64_in >>= 8;
                        if(cnt >= DEF_Outbytes ){
                ...
                for(x = 0; x<DEF_InDivOut; x++){
                        output_valid->push(temp8);
                }

                temp64_in = 0;
                /*temp8 = create_rand(0,2); // Error
                input_error->push(temp8);
                for(x = 0; x<DEF_InDivOut; x++){
                        output_error->push(temp8);
                }*/
                for(x = 0; x<DEF_InDivOut; x++){
                        temp64_out = create_rand(0,(1<<DEF_Outbytes)); // Error
                        output_error->push(temp64_out);
                        temp64_in |= ((temp64_out& bitmask_out )<<(DEF_Inbytes)); 
                        temp64_in >>= DEF_Outbytes; 
                }
                ...
                temp64_in = 0;
                for(x = 0; x<DEF_InDivOut; x++){
                        temp64_out = create_rand(0,(1<<DEF_Outbytes)); // Strobes
...
void print_fifo(std::queue<uint64_t>* input_data, std::queue<uint64_t>* output_data, std::queue<uint8_t>* input_strb, std::queue<uint8_t>* output_strb, std::queue<uint8_t>* input_error, std::queue<uint8_t>* output_error, std::queue<uint8_t>* input_valid, std::queue<uint8_t>* output_valid ){
        ...
        printf("Input Data \tOutput Data \tInput Strb \tOutput Strb \tInput Error \tOutput Error \tInput Valid \tOutput Valid\n");
        uint8_t cnt = DEF_InDivOut;
        uint16_t fifo_size = output_data->size();
        for(uint16_t i=0; i<fifo_size; i++){
                cnt++;
                if( cnt >= DEF_InDivOut ){
                        printf("0x%08lX \t0x%08lX \t0x%02X \t\t0x%02X \t\t0x%02X \t\t0x%02X \t\t0x%02X \t\t0x%02X \n", 
                        input_data->front(), output_data->front(), input_strb->front(), output_strb->front(), input_error->front(), output_error->front(), input_valid->front(), output_valid->front());
                        ...
                        output_data->front(), output_strb->front(), output_error->front(), output_valid->front());
                }
