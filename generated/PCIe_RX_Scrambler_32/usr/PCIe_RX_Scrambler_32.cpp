...
void PCIe_RX_Scrambler_32_TB::soft_scramble(uint8_t* data, uint8_t* isk, uint16_t length, std::ofstream* myfile){
    ...
    *myfile << "{ ";
    for(uint16_t x = 0; x < length; x++){
        ...
        if(data[x] == 0xBC && isk[x] > 0){  // Reset Scrambler
            ...
            if((x+15) < length){
                for(uint8_t p = 6; p<10; p++){
                    if( isk[x+p] == 0 ){
                        if( data[x] != 0x4A && data[x] != 0x45 && data[x] != ~0x4A && data[x] != ~0x45 ){
                            train_true = false;
                        }
                    }
                }
        ...
        // Output Data
        uint8_t output = 0;
        if( (isk[x] > 0) || (train_start <= x && train_start > 0 && (train_start+15) >= x)){    // K Word or Train Set recognized
            output = data[x];
        }else {
