...
void Memory::tick_fifo(){
   /* io_fifo_ready
    ...
    if( fifo_ready_ff > 0 && !tx_fifo_data.empty()){
        tb->m_core->io_fifo_valid = 1;
        //printf("0x%016lX %i\n", tx_fifo_data.front(), tx_fifo_data.empty());
        for(uint8_t i = 0; i<2; i++){
            if(!tx_fifo_data.empty()){
                tb->m_core->io_fifo_bits[(i*2)+0] = tx_fifo_data.front()&0x00000000FFFFFFFF;
                tb->m_core->io_fifo_bits[(i*2)+1] = (tx_fifo_data.front()&0xFFFFFFFF00000000)>>32;  
                tx_fifo_data.pop();
            }else{
                tb->m_core->io_fifo_bits[(i*2)+0] = 0;
                tb->m_core->io_fifo_bits[(i*2)+1] = 0;  
            }
        }  
        
    }else{
        tb->m_core->io_fifo_valid = 0;
    }
    fifo_ready_ff = tb->m_core->io_fifo_ready;
}
